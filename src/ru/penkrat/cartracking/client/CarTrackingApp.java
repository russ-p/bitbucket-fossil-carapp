...
public class CarTrackingApp implements EntryPoint {
	...
	private static CarTrackingApp entryPoint;
	private static String lastView;

	public static CarTrackingApp getInstance() {
		return entryPoint;
	}
	...
	public void onModuleLoad() {
		...
		MenuItem mntmNewPerson = new MenuItem("Person", false, new Command() {
			public void execute() {
				setForm(new PersonEditForm());
			}
		});
		menuBar_1.addItem(mntmNewPerson);

		MenuItem mntmNewViolation = new MenuItem("Traffic violation", false,
				new Command() {
					public void execute() {
						setForm(new ViolationEditForm());
					}
				});
