...
public class CarEditForm extends DockPanel  implements ClickHandler{
	...
	public CarEditForm() {
		...
		lblNewOrEdit.setHorizontalAlignment(HasHorizontalAlignment.ALIGN_CENTER);
		...
		holder.add(new Button("Submit", this));
	...
	private void saveCar(){
		...
		if (tbEngineLiters.getText().isEmpty()){
			Window.alert("'" + tbEngineLiters.getText() + "' is not a valid name.");
			return;
		};
		if (tbRegistrationPlate.getText().isEmpty()){
			Window.alert("'" + tbRegistrationPlate.getText() + "' is not a valid name.");
			return;
		};
		if (!FieldVerifier.isValidVinNumber(tbVinNumber.getText())){
			Window.alert("'" + tbVinNumber.getText() + "' is not a valid VIN Number.");
			return;
		};
